...
def run_step_6():
    ...
    try:
        ...
        add_log("Gemini Search Walk the Talk successful (search-grounded).")
    except Exception as e:
        add_log(f"Gemini Search failed for Walk the Talk: {e}")
        add_log("Falling back to LLM synthesis (not grounded)...")
        result = llm(
            "You are a specialized financial data analyst.",
            user_prompt_walkthetalk(company_name),
            st.session_state.model_step6,
        )
        st.session_state.result_walkthetalk = _SYNTHESIS_DISCLAIMER + result
        st.session_state.concall_source = "llm_synthesis"
        add_log("Walk the Talk generated via LLM synthesis — results are NOT search-grounded.")
    ...
    add_log(f"Saved Walk the Talk to: {full_path} [source: {st.session_state.concall_source}]")
...
def run_step_7():
    ...
    data_source = st.session_state.get("concall_source", "llm_synthesis")
    sp = st.session_state.sp_concall_score
    up = prompt_concall_score(company_name, concall_text, data_source)
