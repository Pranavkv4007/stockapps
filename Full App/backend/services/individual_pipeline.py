...
async def run_individual_pipeline(
    ...
    m1 = models.get("step1", "gemini")
    m2 = models.get("step2", "gemini")
    m3 = models.get("step3", "openai")
    m4 = models.get("step4", "gemini")
    m5 = models.get("step5", "openai")
    m6 = models.get("step6", "gemini")
    m7 = models.get("step7", "openai")
...
async def _step_6(run_id, r, log, model):
    """Walk the Talk via Gemini Search (fallback to LLM)."""
    company_name = r["company_name"]
    ...
    try:
        ...
        log(f"Gemini Search failed: {e}. Falling back to LLM...")
        result = await asyncio.to_thread(
            llm,
        ...
        r["result_walkthetalk"] = result

    file_name = company_name + "_concall.txt"
    full_path = os.path.join(INDIVIDUAL_DIR, file_name)
    with open(full_path, "w", encoding="utf-8") as f:
        f.write(r["result_walkthetalk"])
    log(f"Saved Walk the Talk to: {full_path}")
    return True
...
async def _step_7(run_id, r, log, sp_concall_score, model):
    ...
    up = prompts.prompt_concall_score(company_name, concall_text)
    ...
    with open(full_path, "w", encoding="utf-8") as f:
        f.write(result)
    log(f"Saved Concall Score to: {full_path}")
    return True
